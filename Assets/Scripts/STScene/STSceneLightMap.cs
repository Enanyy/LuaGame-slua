...
public class STSceneLightMap : MonoBehaviour
{
    [System.Serializable]
    struct RendererInfo
    {
    ...
    void Awake()
    {
        if (mRendererInfo == null || mRendererInfo.Length == 0)
            return;

        var lightmaps = LightmapSettings.lightmaps;
        var combinedLightmaps = new LightmapData[lightmaps.Length + mLightmaps.Length];

        lightmaps.CopyTo(combinedLightmaps, 0);
        for (int i = 0; i < mLightmaps.Length; i++)
        {
            combinedLightmaps[i + lightmaps.Length] = new LightmapData();
            combinedLightmaps[i + lightmaps.Length].lightmapColor = mLightmaps[i];
        }

        ApplyRendererInfo(mRendererInfo, lightmaps.Length);
    ...
    static void GenerateLightmapInfo()
    {
        ...
        STSceneLightMap[] prefabs = FindObjectsOfType<STSceneLightMap>();
        ...
        foreach (var instance in prefabs)
        {
            ...
            instance.mRendererInfo = rendererInfos.ToArray();
            instance.mLightmaps = lightmaps.ToArray();
